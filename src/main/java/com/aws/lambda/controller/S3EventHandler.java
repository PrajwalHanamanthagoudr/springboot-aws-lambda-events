...
public class S3EventHandler implements RequestHandler<S3Event, Boolean>{
	...
	public Boolean handleRequest(S3Event input, Context context) {
		...
		logger.log("<------------Inside a method------------>");
		
		//check if are getting any records
		...
		for(S3EventNotification.S3EventNotificationRecord record :input.getRecords()) { 
			...
			//3. processing the InputStream form S3
			
			S3Object s3Object = s3Client.getObject(bucketName, objectKey);
			S3ObjectInputStream inputStream = s3Object.getObjectContent();
			
			//processing CSV file which we uploading in the S3 Bucket
		...
		logger.log("Successfully uploaded the file");
		return true;
	}
